...
class CongressGovAdapter(SourceAdapter):
    ...
    def fetch_member(self, bioguide_id: str) -> RawFetch:
        return self._get(f"member/{bioguide_id}", f"member/{bioguide_id}")
    ...
    def fetch_bill_list(self, congress: int, limit: int = 250, offset: int = 0) -> RawFetch:
        return self._get(f"bill/{congress}", f"bill/{congress}",
                         params={"limit": limit, "offset": offset})

    def fetch_bill_cosponsors(self, congress: int, bill_type: str, bill_number: int) -> RawFetch:
        return self._get(
